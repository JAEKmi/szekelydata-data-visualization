...
DAT.Globe = function(container, opts) {
  ...
  function init() {
    ...
    uniforms['texture'].value = THREE.ImageUtils.loadTexture('img/newOffset.png');
...
//        size = data[i + 2];
  ...
  function addMigrationPaths(data, opts) {
  ...
  function addPath(lat1, lng1, lat2, lng2, size, color, subgeo) {
      ...
      var dir = point2.clone().sub(point1);
      var len = dir.length();
      dir = dir.normalize().multiplyScalar(len * 0.7);
      var mid = point1.clone().add(dir);

      var ctr = new THREE.Vector3(0, 0, 0);
      mid = mid.clone().sub(ctr).normalize().multiplyScalar(200 + size / 2.7);
 ...
 function removeAllPoints() {
  ...
  this.addMigrationPaths = addMigrationPaths;
